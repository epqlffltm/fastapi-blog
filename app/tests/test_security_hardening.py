...
@pytest.mark.parametrize(
    ("model", "payload"),
    [
        (SignUpRequest, {"email": "test@example.com", "password": "가" * 25, "nickname": "tester"}),
        (ResetPasswordVerifyRequest, {"email": "test@example.com", "otp": 123456, "new_password": "가" * 25}),
        (PasswordChangeRequest, {"current_password": "oldpass123", "new_password": "가" * 25, "otp": 123456}),
    ],
)
...
def test_new_password_accepts_exactly_72_utf8_bytes():
    ...
    request = SignUpRequest(email="test@example.com", password=password, nickname="tester")
...
def test_verify_password_treats_overlong_input_as_mismatch(monkeypatch):
    ...
    assert AuthService().verify_password("가" * 25, "$2b$12$fakehashedpassword") is False
...
async def test_hash_password_async_runs_in_threadpool(monkeypatch):
    ...
    monkeypatch.setattr("app.service.auth.run_in_threadpool", fake_run_in_threadpool)
    service = AuthService()
...
def test_login_overlong_password_returns_generic_401(client, mock_user_repo, mock_rate_limit):
    user = _make_user()
    user.password = AuthService().hash_password("password123")
    mock_user_repo.get_user_by_email.return_value = user
    response = client.post("/user/log-in", json={"email": "test@example.com", "password": "가" * 25})
...
def test_sign_up_overlong_password_returns_422(client, mock_user_repo):
    response = client.post("/user/sign-up", json={"email": "test@example.com", "password": "가" * 25, "nickname": "tester"})
...
def test_redis_failure_does_not_block_post_read(client, mock_post_repo, mock_redis):
    post = _make_post()
...
@pytest.mark.asyncio
async def test_otp_verify_and_consume_maps_atomic_script_result(mock_redis, raw_result, expected):
    mock_redis.eval.return_value = raw_result
    service = OTPService(redis=mock_redis)
    result = await service.verify_and_consume("Test@Example.com", 123456, purpose="reset")
    assert result is expected
    script, key_count, otp_key, attempts_key, provided, max_attempts, ttl = mock_redis.eval.await_args.args
...
def test_signup_otp_attempt_limit_returns_429(auth_client, current_user, mock_redis, mock_user_repo):
...
def test_signup_invalid_otp_returns_400(auth_client, current_user, mock_redis, mock_user_repo):
    current_user.is_verified = False
...
def test_password_change_increments_token_version_and_clears_cookie(auth_client, current_user, mock_user_repo, mock_redis):
    service = AuthService()
    ...
    response = auth_client.patch("/user/me/password", json={"current_password": "oldpass123", "new_password": "newpass456", "otp": 123456})
    assert response.status_code == 200
...
def test_password_reset_increments_token_version(client, mock_user_repo, mock_redis):
    user = _make_user()
    user.token_version = 8
    mock_user_repo.get_user_by_email.return_value = user
    mock_redis.eval.return_value = OTPVerifyResult.VERIFIED.value
    response = client.post("/user/password/reset/verify", json={"email": user.email, "otp": 123456, "new_password": "newpassword123"})
    assert response.status_code == 200
...
def test_stale_token_version_is_rejected(client, mock_user_repo):
    ...
    client.cookies.set("access_token", AuthService().create_jwt(user.id, token_version=1))
...
def test_matching_token_version_is_accepted(client, mock_user_repo):
    ...
    user.token_version = 2
    mock_user_repo.get_user_by_id.return_value = user
    client.cookies.set("access_token", AuthService().create_jwt(user.id, token_version=2))
    response = client.get("/user/me")
    assert response.status_code == 200
...
def test_login_issues_current_token_version(client, mock_user_repo, mock_rate_limit):
    service = AuthService()
    user = _make_user()
    user.password = service.hash_password("password123")
    user.token_version = 7
    mock_user_repo.get_user_by_email.return_value = user
    response = client.post("/user/log-in", json={"email": user.email, "password": "password123"})
    assert response.status_code == 200
