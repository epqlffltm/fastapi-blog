...
def test_upload(auth_client, mock_upload_service, mock_upload_repo):
...
def test_upload_records_original_name(auth_client, mock_upload_service, mock_upload_repo):
    """저장 이름은 UUID 지만 원본 이름은 기록에 남는다"""
    mock_upload_service.save.return_value = ("abc123.png", 1234)

    auth_client.post("/upload", files={"file": ("사진.png", b"x", "image/png")})
